  ...
  <card class="card" title="Add Project">
    <div>
      <form @submit.prevent>
        <div class="row">
          ...
          <div class="col-md-3">
            ...
            <select id="priority" class="form-control">
              <option
                v-for="ClintName in user.ClientNames"
                v-bind:value="ClintName.Key"
              >{{ ClintName.Value }}</option>
            </select>
          </div>
        </div>
        ...
        <div class="row">
          <div class="col-md-6">
            <fg-input type="text" label="Site Location" placeholder="Location" v-model="user.site"></fg-input>
          </div>
        </div>

        <div class="text-center">
          <p-button type="info" round @click.native.prevent="AddProject">Add Project</p-button>
        </div>
...
export default {
  ...
  methods: {
    updateProfile() {
      this.$http.get(companyRef + ".json").then(
        response => {
          // get body data
    ...
    AddProject: function() {
      this.$http
        .post(projectRef + ".json", {
          name: this.user.prjectName,
          site: this.user.site,
        ...
        .then(
          response => {
            ...
            //console.log("keeeri ");
            userRef
              .child(this.$store.state.userKey)
              .child("projects")
              .push(response.body["name"]);
          },
          response => {
            // error callback
          }
