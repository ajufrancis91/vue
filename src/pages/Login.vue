      ...
      <v-container class="fill-height" fluid>
        <v-row align="center" justify="center">
          <v-col cols="12" sm="8" md="4">
            <v-card class="elevation-12" v-if="!hidelogin">
              ...
              <v-card-text>
                <v-form>
                  <v-text-field v-model="username" label="Email Address" name="login" type="text"></v-text-field>
                ...
                snapshot.forEach(data => {
                  if (
                    data
                      .child("password")
                      .exportVal()
                      .toString() === vm.password
                  ) {
                    console.log("success");
                    vm.invalid = false;
                    vm.$store.state.username = data
                      .child("name")
                      .exportVal()
                      .toString();
                    vm.$store.state.email = vm.username;
                    vm.$store.state.companyCode = data
                      .child("companyCode")
                      .exportVal()
                      .toString();
                    vm.$store.state.userKey = data.key;
                    vm.$store.state.securityGroups = data
                      .child("securityGroups")
                      .exportVal()
                      .toString();
